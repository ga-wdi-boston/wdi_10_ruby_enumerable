...
class MyEnumeration
  ...
  def elements_ending_in_er
    collection.find_all {|x| x.end_with?'er'}
  end
  ...
  def index_of_first_awesome_element
    collection.find_index {|i| i[:awesome]}
  end
  ...
  def sum_of_experiences
    collection.inject(0) { |sum, n| sum + n[:years_experience] }
  end
  ...
  def longest_element_name_using_inject
    collection.inject {|memo, word| memo[:name].length > word[:name].length ? memo : word}
  end
