...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.count { |i| i.is_a? Fixnum } + collection.count { |i| i.is_a? Float }
  end
  ...
  def square_numbers
    collection.map { |integer| integer ** 2 }
  end
  ...
  def element_frequency_count
    collection.each_with_object(Hash.new(0)) { |word, hash| hash[word] += 1  }
  end
  ...
  def elements_ending_in_er
    collection.find_all { |element| element.include? "er" }
  end
  ...
  def index_of_first_awesome_element
    collection.find_index { |i| i[:awesome] == true }
  end
  ...
  def longest_element_name_using_inject
    collection.inject do |memo, hash|
      if memo[:name].length > hash[:name].length
        memo
      else
        hash
      end
