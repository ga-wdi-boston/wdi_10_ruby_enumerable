...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.select{|element| element.class == Float || element.class == Fixnum}.count
  end
  ...
  def sum_of_experiences
    collection.reduce(0){|sum, hash| sum += hash[:years_experience] }
  end
  ...
  def longest_element_name_using_inject
    collection.inject {|hash1, hash2| hash1[:name].length > hash2[:name].length ? hash1 : hash2}
  end
  ...
  def elements_with_longest_and_shortest_names
    collection.minmax_by { |hash| hash[:name].length}
  end
  ...
  def separate_elements_that_like_functional_programming_from_rest
    collection.partition {|hash| hash[:likes_functional_programming]}
  end

  # Refer to reject
  # http://ruby-doc.org/core-2.1.4/Enumerable.html#method-i-reject
  def elements_who_dislike_functional_programming
    collection.reject {|hash| hash[:likes_functional_programming]}
  end
  ...
  def elements_sorted_by_experience
    collection.sort {| years1, years2 | years1[:years_experience] <=> years2[:years_experience]}
  end
