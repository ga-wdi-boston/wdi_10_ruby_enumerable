...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.group_by(&:class)[Fixnum].count + collection.group_by(&:class)[Float].count
  end
  ...
  def capitalize_words
    collection.collect(&:capitalize)
  end

  def square_numbers
    collection.collect { |x| x*x }
  end
  ...
  def find_first_awesome_person
    collection.detect { |hash| hash[:awesome] }
  end
  ...
  def drop_until_its_hot
    #snoop = []
    collection.drop_while { |snoop| collection.index(snoop) < collection.index('hot')}
  end
  ...
  def groups_of(num)
    collection.each_slice(num).to_a {|pizza| pizza}
  end
  ...
  def element_frequency_count
    collection.each_with_object(Hash.new(0)) {|key, hash| hash[key] += 1 } # I want to follow up to see how that '+= 1' works
  end
  ...
  def elements_ending_in_er
    collection.find_all {|x| x.include?("er")}
  end
  ...
  def longest_element_name_using_inject
    collection.inject { |memo, name| memo[:name].length > name[:name].length ? memo : name }
  end
  ...
  def separate_elements_that_like_functional_programming_from_rest
    collection.partition {|like| like[:likes_functional_programming]}
  end

  # Refer to reject
  # http://ruby-doc.org/core-2.1.4/Enumerable.html#method-i-reject
  def elements_who_dislike_functional_programming
    collection.reject {|dislike| dislike[:likes_functional_programming]}
    #collection.select {|like| like[:likes_functional_programming] == false} #another option
  end
