...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.select{|x| x.class == Float || x.class == Fixnum}.count
  end
  ...
  def positive_and_negative_numbers #Not sure what this is supposed to do.
    collection.flat_map{|x| [x,-x].sort{|a,b| b <=> a}}
  end
  ...
  def drop_until_its_hot #Har har har
    collection.drop_while{|it| it != 'hot'} 
  end
  ...
  def element_frequency_count
    collection.each_with_object(Hash.new(0)){|word,histogram| histogram[word] += 1 || 0 } #This one was new for me - it's interesting.
  end
  ...
  def elements_ending_in_er
    collection.select{|word| word[-2,2] == "er"}
  end
  ...
  def sum_of_experiences
    collection.reduce(0) {|sum,person| sum + person[:years_experience] } 
  end
  ...
  def longest_element_name_using_inject
    collection.reduce {|memo, person| memo[:name].length > person[:name].length ? memo : person }
  end
  #This one and the previous were the trickiest ones so far, I think - so much syntax to mess up.
  ...
  def elements_sorted_by_experience
    collection.sort{|person_a, person_b| person_a[:years_experience] <=> person_b[:years_experience]}
  end
