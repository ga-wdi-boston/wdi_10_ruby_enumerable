...
class MyEnumeration
  ...
  def positive_and_negative_numbers
    collection.flat_map {|int| [int.abs, -int.abs]}
  end
  ...
  def element_frequency_count
    ...
    # Did not know how to solve this one
  end
  ...
  def elements_ending_in_er
    collection.find_all {|ele| ele.end_with? 'er'}
  end
  ...
  def elements_sorted_by_experience
    collection.sort {|exp1, exp2| exp1[:name][:years_experience] <=> exp2[:name][:years_experience]}
  end
