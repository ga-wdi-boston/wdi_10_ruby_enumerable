...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.select { |item| item.class == Fixnum || item.class == Float }.count
  end
  ...
  def square_numbers
    collection.collect { |item| item ** 2 }
  end
  ...
  def find_first_awesome_person
    collection.find {| hash | hash[:awesome] }
  end
  ...
  def groups_of(num)
    collection.each_slice(num).map { |a| a }
  end
  ...
  def elements_sorted_by_experience
    # collection.sort { |hash| hash[:years_experience] }
    # (1..10).sort { |a, b| b <=> a }
