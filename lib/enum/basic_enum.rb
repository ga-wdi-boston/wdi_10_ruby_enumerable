...
class MyEnumeration
  ...
  def find_first_awesome_person
   collection.detect{ |hash| hash[:awesome] == true }
  end
  ...
  def sum_of_experiences
    collection.inject(0){ |sum, hash| sum + hash[:years_experience] }
  end
  ...
  def longest_element_name_using_inject
    collection.inject{ |memo, element| memo[:name].length > element[:name].length ? memo : element }
  end
  ...
  def separate_elements_that_like_functional_programming_from_rest
    collection.partition{ |element| element[:likes_functional_programming] == true }
  end

  # Refer to reject
  # http://ruby-doc.org/core-2.1.4/Enumerable.html#method-i-reject
  def elements_who_dislike_functional_programming
    collection.reject{ |element| element[:likes_functional_programming] == true }
  end
