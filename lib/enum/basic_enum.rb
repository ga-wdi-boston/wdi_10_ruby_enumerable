...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.count{|x| (x.is_a? Fixnum) || (x.is_a? Float)}
  end
  ...
  def element_frequency_count
    collection.each_with_object({}){|x,a| a[x] = collection.count(x)}
  end
  ...
  def sum_of_experiences
    collection.map{|x| x[:years_experience]}.inject(:+)
  end
  ...
  def longest_element_name_using_inject
    collection.inject{|memory, word| memory[:name].length > word[:name].length ? memory : word}
  end
