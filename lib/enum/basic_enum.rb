...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.count{ |element| (element.is_a? Float) || (element.is_a? Fixnum)}
  end
  ...
  def contains_a_word_longer_than?(min_length)
    collection.map? { |word| word.length > min_length }
  end

  def capitalize_words
    collection.map(&:capitalization)
  end
  ...
  def group_elements_by_favorite_language
    collection.group_by { |name| name[:favorite_language] }
  end
  ...
  def sum_of_experiences
    collection.map{ |name| name[:years_experience] }.inject(:+)
  end
  ...
  def longest_element_name_using_inject
    collection.map { |name| name[:name].length}.inject(:+)
  end
  ...
  def separate_elements_that_like_functional_programming_from_rest
    collection.partition { |name| name[:like_functional_programming]}
  end

  # Refer to reject
  # http://ruby-doc.org/core-2.1.4/Enumerable.html#method-i-reject
  def elements_who_dislike_functional_programming
    collection.reject { |name| name[:dislike_functional_programming]}
  end
  ...
  def elements_sorted_by_experience
    collection.sort { |name| name[:by_experience]}
  end
