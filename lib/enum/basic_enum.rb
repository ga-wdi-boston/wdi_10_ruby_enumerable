...
class MyEnumeration
  ...
  def contains_a_word_longer_than?(min_length)
    collection.any? { |string| string.length > min_length }
  end
  ...
  def groups_of(num)
    collection.each_slice(num).map { |a| p a }.to_a
  end
  ...
  def sum_of_experiences
    collection.inject(0) { |sum, exp| sum + exp[:years_experience]}
  end
  ...
  def most_experienced_element
    collection.max_by { |exp| exp[:years_experience] }
  end
  ...
  def separate_elements_that_like_functional_programming_from_rest
    collection.partition { |name| name[:likes_functional_programming] }
  end

  # Refer to reject
  # http://ruby-doc.org/core-2.1.4/Enumerable.html#method-i-reject
  def elements_who_dislike_functional_programming
    collection.reject { |name| name[:likes_functional_programming] }
  end
